...
| `thresholds` | Specifies an object of threshold values at which the gauge's color changes. Checkout an example [here](#configure-threshold-color-ranges). Reactivity is identity-based — replace the object reference (immutable update) to trigger a redraw; in-place mutation is not detected.  | No |  `none` | {}
| `markers` | Specifies an object of marker values at which value to place a marker. Can be a line or triangle and optionally may specify color, size and label. Note if you use labels you should add to margin value to the gauge using the margin option. Reactivity is identity-based — replace the object reference to trigger a redraw. | No | A special object of the format shown. Currently type supports "line" or "triangle" marker. ```{ "50": { color: "#555", type: "triangle", size: 8, label: "Goal", font: "12px arial" } , ... }``` | {} |
